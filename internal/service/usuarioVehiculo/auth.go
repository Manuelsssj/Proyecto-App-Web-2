...
const (
	secretoPorDefecto  = "rideUleam-secreto-solo-dev"
...
func NuevoAuthService(repo storage.UsuarioRepository, opts ...AuthOption) *AuthService {
	s := &AuthService{
		repo:     repo,
...
func (s *AuthService) Registrar(email, password string, rolOpcional ...string) (models.Usuario, error) {
	...
	rol := "pasajero"
	if len(rolOpcional) > 0 {
		rol = normalizarRol(rolOpcional[0])
	}
